...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        conv_H = 1 + (input_dim[1] + conv_pad * 2 - filter_size) // 1
        conv_W = 1 + (input_dim[2] + conv_pad * 2 - filter_size) // 1
        pool_H = (conv_H - 2) // 2 + 1
        pool_W = (conv_W - 2) // 2 + 1
    ...
    def loss(self, X, y=None):
        ...
        conv_out = conv_out.reshape((X.shape[0], -1))
        ...
        dhiden, grads['W3'], grads['b3'] = affine_backward(dscores, scores_cache)
        dconv, grads['W2'], grads['b2'] = affine_relu_backward(dhiden, hidden_cache)
        _, grads['W1'], grads['b1'] = conv_relu_pool_backward(dconv.reshape(conv_out_shape), conv_cache)
